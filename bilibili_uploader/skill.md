...
| `source` | string | `""` | Source URL for repost videos (**required when `no_reprint=0`**) |

> ⚠️ **Important**: When uploading reposted content (`no_reprint=0`), the `source` parameter is **required** by Bilibili API. Without it, the upload will fail with error "稿件类型为转载时，转载来源不能为空".

#### Examples

```bash
# Upload and publish (original content)
python main.py publisher upload '{"file_path": "./video.mp4", "title": "My Video", "description": "Hello World", "tags": ["test", "demo"], "category": "171", "no_reprint": 1}'

# Upload repost content (requires source URL)
python main.py publisher upload '{"file_path": "./video.mp4", "title": "Reposted Video", "description": "Reposted from YouTube", "tags": ["repost"], "no_reprint": 0, "source": "https://www.youtube.com/watch?v=VIDEO_ID"}'
...
# Upload repost content (source is required)
result = await app.execute("publisher", "upload",
    file_path="./video.mp4",
    title="Reposted Video",
    description="Reposted from YouTube",
    tags=["repost"],
    no_reprint=0,
